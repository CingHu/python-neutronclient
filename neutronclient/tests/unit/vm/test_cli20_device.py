...
class CLITestV20VmDeviceJSON(test_cli20.CLITestV20Base):
    _RESOURCE = 'device'
    _RESOURCES = 'devices'

    def setUp(self):
        plurals = {'devices': 'device'}
        super(CLITestV20VmDeviceJSON, self).setUp(plurals=plurals)

    def test_create_device_all_params(self):
        cmd = device.CreateDevice(test_cli20.MyApp(sys.stdout), None)
        my_id = 'my-id'
    ...
    def test_list_devices(self):
        cmd = device.ListDevice(test_cli20.MyApp(sys.stdout), None)
    ...
    def test_show_device_id(self):
        cmd = device.ShowDevice(test_cli20.MyApp(sys.stdout), None)
    ...
    def test_update_device(self):
        cmd = device.UpdateDevice(test_cli20.MyApp(sys.stdout), None)
        my_id = 'my-id'
        key = 'new-key'
        value = 'new-value'
    ...
    def test_delete_device(self):
        cmd = device.DeleteDevice(test_cli20.MyApp(sys.stdout), None)
        my_id = 'my-id'
        args = [my_id]
        self._test_delete_resource(self._RESOURCE, cmd, my_id, args)


class CLITestV20VmDeviceXML(CLITestV20VmDeviceJSON):
    format = 'xml'
